<?php
 ...
 if($_SERVER ['REQUEST_METHOD'] == 'GET'){
    ...
    $obj = new operation();
    $result1 = $obj->getData('category',"*",['name'=> $productCategory]);
    ...
    $valid = $fileobj->fileValidation($ext,$validext);
           ...
           if(!empty($productName)&&!empty($productPrice)&&$valid){
                          $dir=$fileobj->file_upload($filename,$tempname,$dirname,$productName);
                           $data =['name'=>$productName,'category'=>$cat_id,'price'=>  $productPrice,'image'=>$dir];
                          // $dirname = 'regs';
   ...
   <div class="container">
       <div class="row">
           <div class="col-sm-3">
               ...
               <input class="btn btn-primary" type="submit" name="create" value="Update-Product">
           </div>
       </div>
   </div>
